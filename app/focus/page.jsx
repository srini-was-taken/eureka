...
export default function FocusPage() {
  ...
  async function handleFile(file) {
    ...
    if (res.ok) {
      ...
      if (userId) loadPreviousPdfs(userId);
    }
    setUploading(false);
    setStage("session");
  }
  ...
  return (
    <div style={{ height: "100vh", background: BG, color: TEXT, display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "10px 24px", borderBottom: `1px solid ${BORDER}`, display: "flex", alignItems: "center", gap: 12, background: CARD, flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <Btn small variant={mode === "highlight" ? "primary" : "ghost"} onClick={() => setMode(m => m === "highlight" ? null : "highlight")}>🖊 {mode === "highlight" ? "Highlighting..." : "Highlight"}</Btn>
          <Btn small variant={mode === "note" ? "outline" : "ghost"} onClick={() => setMode(m => m === "note" ? null : "note")}>🗒 {mode === "note" ? "Click PDF..." : "Note"}</Btn>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <div style={{ width: 260, borderLeft: `1px solid ${BORDER}`, display: "flex", flexDirection: "column", background: CARD, flexShrink: 0 }}>
          <div style={{ display: "flex", borderBottom: `1px solid ${BORDER}`, flexShrink: 0 }}>
            {[{ key: "stats", label: "Stats" }, { key: "highlights", label: `🖊 ${highlights.length}` }, { key: "notes", label: `🗒 ${notes.length}` }, { key: "flashcards", label: `✦ ${flashcards.length}` }].map(tab => (
          ...
          <div style={{ flex: 1, padding: 16, overflowY: "auto", display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <>
                {[["Time Spent", fmt(25 * 60 - timer)], ["Highlights", highlights.length], ["Notes", notes.length], ["Flashcards", flashcards.length], ["Pages", totalPages]].map(([k, v]) => (
                  <div key={k} style={{ display: "flex", justifyContent: "space-between", fontSize: 13, padding: "10px 14px", background: CARD2, borderRadius: 10 }}>
                    <span style={{ color: MUTED }}>{k}</span><span style={{ fontWeight: 700, color: TEAL }}>{v}</span>
                  </div>
                ))}
                ...
                {highlights.length === 0
                  ...
                  : highlights.map((h, i) => (
                    <div key={h.id} style={{ background: "#fef08a12", border: "1px solid #fef08a30", borderRadius: 10, padding: "10px 12px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <span style={{ fontSize: 12, color: "#fef08a" }}>Highlight · Page {h.pageNum || h.page}</span>
                ...
                {notes.length === 0
                  ...
                  : notes.map(n => (
                    <div key={n.id} onClick={() => setActiveNote(activeNote?.id === n.id ? null : n)}
                      ...
                      <div style={{ fontSize: 11, color: TEAL, fontWeight: 700, marginBottom: 4 }}>Page {n.pageNum || n.page}</div>
            ...
            {activeTab === "flashcards" && (
              <>
                ...
                {flashcards.length === 0
                  ? <div style={{ textAlign: "center", color: MUTED, fontSize: 13, marginTop: 16 }}>No flashcards yet.</div>
                  : flashcards.map(card => (
                    <div key={card.id} onClick={() => setFlippedCards(f => ({ ...f, [card.id]: !f[card.id] }))}
                      ...
                      <div style={{ fontSize: 10, color: MUTED, fontWeight: 700, marginBottom: 6, letterSpacing: 0.5 }}>{flippedCards[card.id] ? "ANSWER" : "QUESTION"} · tap to flip</div>
                      <p style={{ fontSize: 13, color: flippedCards[card.id] ? TEAL : TEXT, lineHeight: 1.5, margin: 0 }}>{flippedCards[card.id] ? card.back : card.front}</p>
